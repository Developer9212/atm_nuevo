...
begin
  ...
  from   tablas
  where  idtabla = 'cajero_receptor_idcajero_idusuario' and idelemento = p_idcajero;
  if not found then
    raise notice 'NO EXISTE cajero_receptor_idcajero_idusuario / % RELACIONE IDCAJERO CON IDUSUARIO SAICOOP', p_idcajero;
    return NULL;
  end if;
...
begin
  ...
  i_cont := sai_temporal_procesa(p_idusuario, p_sesion, p_fecha, i_origenapl, i_idpoliza, 3, t_concepto, FALSE, TRUE);
