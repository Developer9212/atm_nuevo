...
drop table if exists cajero_receptor_movimientos_ca cascade;
create table cajero_receptor_movimientos_ca (
  n_mov           integer,
  fecha           timestamp,
  idusuario       integer,
  sesion          varchar,
  ...
  idgrupo         integer,
  idsocio         integer,
  idorigenp       integer default 0,
  idproducto      integer default 0,
  idauxiliar      integer default 0,
...
drop trigger if exists t_i_cajero_receptor_movimientos_ca on cajero_receptor_movimientos_ca;
create or replace function t_i_cajero_receptor_movimientos_ca() returns trigger as $$
declare
  r_prod  record;
  r_aux   record;
begin
  if tg_op = 'INSERT' then
    ...
    if r_prod.tipoproducto in (0,1,2,4,5,8) then
      ...
      if not found then
        raise exception 'Error: el O-P-A que se esta registrando en la tabla "cajero_receptor_movimientos_ca" no es valido.';
      end if;
    end if;
  end if;
...
create trigger t_i_cajero_receptor_movimientos_ca
before         insert
on             cajero_receptor_movimientos_ca
for each row   execute procedure t_i_cajero_receptor_movimientos_ca();

drop table if exists cajero_receptor_movimientos_spei cascade;
create table cajero_receptor_movimientos_spei (
...
Llenar la siguiente tabla con la relacion cajero_receptor <--> idusuario

idtabla                             idelemento                    dato1
==================================  ============================  ====================
cajero_receptor_idcajero_idusuario  (N/Serie cajero o ID cajero)  (idusuario saicoop)


idtabla:    cajero_receptor
idelemento: horario_actividad
dato1:      (hora inicial) Ej: 08:00
dato2:      (hora final)   Ej: 23:00
dato3:      (dias operables sep.pipa(|)) Ej: 0:Dom, 1:Lun, ...6:Sab

idtabla:    cajero_receptor
idelemento: prefijo_concepto_poliza
dato2:      (Prefijo concepto particular)
            Nota: En caso de NULL o vacio, por default es: "Movimientos de Cajero Receptor"
                  El Sufijo o depues de: " Ref: " + (Num.Ref definida)
dato3:      (dias operables sep.pipa(|)) Ej: 1|2|3|4|5|6  (0:Domingo x si no se abre)
            Nota: Definicion por numero, Ej: 0:Dom, 1:Lun, ...6:Sab


***** EN CASO DE TDD  *****************************************************
idtabla:    cajero_receptor
idelemento: producto_tdd
dato1:      (idproducto)

***** EN CASO DE SPEI *****************************************************
idtabla:    cajero_receptor
idelemento: cuenta_spei
dato3:      (idorigen de aplicacion)

idtabla:    cajero_receptor
idelemento: spei_horario_actividad
dato1:      (hora inicial) Ej: 08:00
dato2:      (hora final)   Ej: 23:00
dato3:      (dias operables sep.pipa(|)) Ej: 0:Dom, 1:Lun, ...6:Sab
----------------------------------- <<<<< FIN: CONFIGURACIONES >>>>> -------------------------------------
...
----------------------------------- <<<<< NOTAS O EXPLICACION DE USO >>>>> -------------------------------------

- Llenar la tabla: "cajero_receptor_movimientos_ca" con datos, balanceando entre cargos y abonos
- Para determinar el idusuario SAICoop correspondiente al cajero en uso, usar la funcion:
    x_idusuario := sai_cajero_receptor_idusuario_de_idcajero (varchar)  (ID. Cajero Receptor)
- Para aplicar los movimientos insertados,  usar la funcion:
   sai_cajero_receptor_aplica_transaccion (integer,varchar,varchar)
    1) p_idusuario
    2) p_sesion
    3) p_referencia

----------------------------------- <<<<< FIN: NOTAS O EXPLICACION DE USO >>>>> -------------------------------------

*/
create or replace function
sai_cajero_receptor_idusuario_de_idcajero (varchar) returns integer as $$
declare
p_idcajero    alias for $1;
  i_idusuario   integer;
  r_paso        record;
begin
select
into   i_idusuario dato1::integer
from   tablas
where  idtabla = 'cajero_receptor_identificacion' and idelemento = p_idcajero;
if not found then
    raise notice 'NO EXISTE cajero_receptor_identificacion / % RELACIONE IDCAJERO CON IDUSUARIO SAICOOP', p_idcajero;
return NULL;
end if;
...
if not found then
    raise notice 'IDUSUARIO % NO EXISTE O ESTA INACTIVO', i_idusuario;
return NULL;
end if;
...
create or replace function
sai_cajero_receptor_spei_rechazado (integer,varchar,varchar) returns numeric as $$
declare
...
for r_spei
  in  select *
      from   cajero_receptor_movimientos_spei
      where  idorden_spei = p_idorden_spei
          loop
      insert
      into   cajero_receptor_movimientos_ca
...
create or replace function
sai_cajero_receptor_monto_adelanto_interes (integer,integer,integer,date,text) returns numeric as $$
declare
r_aux         record;
  t_adela       text;
  t_aux         text;
...
create or replace function
sai_cajero_receptor_limite_adelanto (integer,integer,integer,date,numeric,text) returns numeric as $$
declare
r_aux         record;
  t_distr       text;
  t_adela       text;
...
create or replace function
sai_cajero_receptor_servicio_activo_inactivo () returns boolean as $$
declare
...
from   tablas
where  idtabla = 'cajero_receptor' and idelemento = 'horario_actividad';

if sai_findstr(t_hora_ini,':') = 0 or sai_findstr(t_hora_fin,':') = 0 or
     split_part(t_hora_ini,':',1) > split_part(t_hora_fin,':',1)
  then
    raise notice 'MAL DEFINIDAS LAS HORAS EN LA TABLA: cajero_receptor / horario_actividad';
return NULL;
end if;
...
create or replace function
sai_cajero_receptor_servicio_activo_inactivo_spei () returns boolean as $$
declare
d_fecha_servidor  date;
  d_fecha_origenes  date;
  b_estatus_oper    boolean;
...
into   t_hora_ini, t_hora_fin, t_dow_operables dato1, dato2, dato3
from   tablas
where  idtabla = 'cajero_receptor' and idelemento = 'spei_horario_actividad';

if sai_findstr(t_hora_ini,':') = 0 or sai_findstr(t_hora_fin,':') = 0 or
     split_part(t_hora_ini,':',1) > split_part(t_hora_fin,':',1)
  then
    raise notice 'MAL DEFINIDAS LAS HORAS EN LA TABLA: cajero_receptor / spei_horario_actividad';
return NULL;
end if;
...
create or replace function
sai_cajero_receptor_termina_transaccion (date,integer,varchar,varchar) returns integer as $$
declare
p_fecha       alias for $1;
...
from   origenes
  ...
  if p_idusuario is NULL then
    raise notice 'ERROR: EL IDUSUARIO ESTA EN NULL';
return NULL;
end if;
...
select
into   r_paso *
from   cajero_receptor_movimientos_ca
...
create or replace function
sai_cajero_receptor_detalle_transaccion_aplicada (date,integer,varchar,varchar) returns varchar as $$
declare
r_movs    record;
  n_montos  numeric[];
begin
...
--drop function if exists sai_cajero_receptor_aplica_movs (date,integer,varchar,varchar);
create or replace function
sai_cajero_receptor_aplica_movs (date,integer,varchar,varchar,integer,boolean) returns integer as $$
declare
p_fecha           alias for $1;
  p_idusuario       alias for $2;
  p_sesion          alias for $3;
...
into   r_tab *
from   tablas
where  idtabla = 'cajero_receptor' and idelemento = 'prefijo_concepto_poliza' and
    dato2 is not NULL and dato2 != '';
if not found then
    raise notice 'WARNING: TABLA cajero_receptor / prefijo_concepto_poliza NO ESTA DEFINIDIA.'
                 'SE USARA UN CONCEPTO DE POLIZA GENERICO';
    t_pre_concepto := 'Movimientos de Cajero Receptor';
else
  ...
  if i_idpoliza = 0 then
    i_idpoliza := sai_folio(TRUE,t_llave_folio_poliza);
    t_concepto := t_pre_concepto||' Ref: '||p_referencia;
end if;

update usuarios
set    pingreso = p_fecha::text||'|'||i_origenapl::text||'|1|'||i_idpoliza::text,
         ticket   = ticket + 1
where  idusuario = p_idusuario;

i_cont := sai_temporal_procesa(p_idusuario, p_sesion, p_fecha, i_origenapl, i_idpoliza, 1, t_concepto, FALSE, TRUE);
...
update cajero_receptor_movimientos_ca
set    aplicado = TRUE
where  idusuario = p_idusuario and sesion = p_sesion and date(fecha) = p_fecha and referencia = p_referencia;
else
    raise exception 'sai_cajero_receptor_aplica_movs: HUBO UN ERROR AL APLICAR LA FUNCION SAI_TEMPORAL_PROCESA';
end if;
...
create or replace function
sai_cajero_receptor_procesa_movs (_text) returns integer as $$
declare
p_mat               alias for $1;
  p_idusuario         integer;
  p_sesion            varchar;
...
from   tablas
    where  idtabla = 'cajero_receptor' and idelemento = 'cuenta_spei';
end if;
...
create or replace function
sai_cajero_receptor_prestamo_cuanto(integer,integer,integer,date,integer,text)
returns text as $$
declare
...
create or replace function
sai_cajero_receptor_prestamo_adelanto_exacto (integer,integer,integer,date,numeric,numeric) returns numeric as $$
declare
p_idorigenp       alias for $1;
  p_idproducto      alias for $2;
  p_idauxiliar      alias for $3;
...
create or replace function
sai_cajero_receptor_desglosa_prestamo (date,integer,varchar,varchar) returns integer as $$
declare
p_fecha               alias for $1;
  p_idusuario           alias for $2;
  p_sesion              alias for $3;
...
b_hay_adelint := found;
  ...
  raise notice '-----------------------------------------------------------------------------------------------';
  raise notice ' ';
  raise notice 'SE COMENZARA A APLICAR MOVIMIENTOS cajero_receptor:';
...
from   tablas
where  idtabla = 'cajero_receptor' and idelemento = 'producto_tdd';
if not found then
    i_prod_tdd := 0;
end if;
...
for r_mov
  in  select   mca.*, a.tipoamortizacion as ta, a.tasaio, a.idorigen, a.idgrupo, a.idsocio, a.saldo
      from     cajero_receptor_movimientos_ca mca
...
raise notice ' ';
    ...
    t_cuanto := sai_cajero_receptor_prestamo_cuanto(r_mov.idorigenp, r_mov.idproducto, r_mov.idauxiliar, p_fecha, r_mov.ta, t_aux);
...
where  fecha::date = date(r_mov.fecha) and idusuario = r_mov.idusuario and sesion = r_mov.sesion and referencia = r_mov.referencia and
          ...
          if n_monto_mov > n_suma_total then
            n_sobrante := sai_cajero_receptor_prestamo_adelanto_exacto (r_mov.idorigenp, r_mov.idproducto, r_mov.idauxiliar,
...
*/
create or replace function
sai_cajero_receptor_aplica_transaccion (integer,varchar,varchar) returns integer as $$
declare
p_idusuario       alias for $1;
  p_sesion          alias for $2;
  p_referencia      alias for $3;
  d_fecha           date;
  r_mov             record;
  r_prod            record;
  r_paso            record;
...
into   r_paso *
from   usuarios
where  idusuario = p_idusuario and activo;
if not found then
    raise notice 'USUARIO NO EXISTE O ESTA INACTIVO';
return NULL;
end if;
...
return -1;
  ...
  i_resp := sai_cajero_receptor_desglosa_prestamo (d_fecha, i_idusuario, p_sesion, p_referencia);
...
t_sai_aux := '';
    ...
    i_movs := sai_cajero_receptor_procesa_movs (t_dim);
end loop;

  if i_movs > 1 then

    i_resp := sai_cajero_receptor_aplica_movs (d_fecha,i_idusuario,p_sesion,p_referencia,i_idorden_spei,b_spei_cancelado);
...
insert
into   cajero_receptor_movimientos_spei
