...
public class ServiceGeneralSpring {
			...
			if(tbAtm != null) {
				...
				if(cuenta != null) {
					...
					if(producto.getTipoproducto() == 0 || producto.getTipoproducto() == 2) {
						...
						if(request.getTipoCuenta() == 1 || request.getTipoCuenta() == 0) {/****************************/
    ...
    public Comprobante depositoTX(TxDeposito objeto) {
    	...
    	try {
			...
			if(tbAtm != null) {
				Integer idUsuario = Integer.parseInt(tbAtm.getDato1());
							...
							if (cuenta_depositar.getEstatus() == 2) {
								...
								/*if(bandera_cambio && objeto.getTipoTX() == 4) {
									...
									if (Integer.parseInt(tot_movs) > 0) {
										...
										tbAtm = tablasService.buscarPorId(tbPk);

										String url = tbAtm.getDato2();
										...
										String mensaje = tbAtm.getDato2().replace("@nombreproducto@", producto.getNombre()).replace("@monto@", String.valueOf(objeto.getMontoDeposito())).replace("@folio@", objeto.getReferencia()).replace("@fecha@", fechaParse);
